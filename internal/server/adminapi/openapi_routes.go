...
var routeDocs = map[string]routeDoc{
	...
	"POST /api/admin/v1/groups/{id}":                      {Tag: "Groups", Summary: "Update a group", Request: groupRequest{}, Response: groupJSON{}, Held: true, Description: "A group's kind can't change. With approvals on, changing the rule of a dynamic group with code assigned to it waits."},
	...
	"POST /api/admin/v1/groups/{id}/members":              {Tag: "Groups", Summary: "Add a device to a static group", Request: memberRequest{}, Status: 204, Held: true, Description: "With approvals on, adding a device to a group with code assigned waits once the group would hold more devices than the threshold."},
	...
	"POST /api/admin/v1/scripts/{id}":         {Tag: "Scripts", Summary: "Update a script, making a new version if its code changed", Request: scriptRequest{}, Response: scriptJSON{}, Held: true, Description: "With approvals on, a new version of a script assigned beyond the threshold is stored but waits for approval before devices receive it."},
	...
	"POST /api/admin/v1/profiles/{id}":         {Tag: "Profiles", Summary: "Update a configuration profile, making a new version", Request: profileRequest{}, Response: profileJSON{}, Held: true, Description: "With approvals on, a new version of a profile assigned beyond the threshold is stored but waits for approval before devices receive it."},
	...
	"POST /api/admin/v1/apps/{id}":         {Tag: "Apps", Summary: "Update an app, making a new version", Request: appRequest{}, Response: appJSON{}, Held: true, Description: "With approvals on, a new version of an app assigned beyond the threshold is stored but waits for approval before devices receive it."},
	...
	"GET /api/admin/v1/server":                      {Tag: "Server", Summary: "The server's version, the newest verified release, and any update in progress", Response: serverJSON{}},
	"POST /api/admin/v1/server/update":              {Tag: "Server", Summary: "Update the server to the newest verified release", Request: serverUpdateRequest{}, Status: 202, Response: serverUpdateResponse{}, Description: "Admins at a console only. The version must be the newest verified release and newer than the running one; the server is never downgraded. With approvals on, the request is held for a second administrator (202 with the approval). The updater verifies the release itself, backs up the database, installs the new build and rolls back if it does not report ready. 409 when there is no updater, an update is running, or the version is refused."},
