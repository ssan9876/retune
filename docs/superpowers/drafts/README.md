...
| [defender-firewall](defender-firewall-spec.md) | M15 | Defender and firewall status in inventory, a `defender` setting, compliance rules | only five Defender preferences, no exclusions or ASR | nothing |
...
Two drafts from that night are not here:
